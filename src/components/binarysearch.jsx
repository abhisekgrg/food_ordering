...
const Binarysearch = () => {
  ...
  const fetchData = () => {
    axios
      .get('http://localhost/php-rest-api/api-fetch-all.php', {
        params: {
          min_price: minPrice,
          max_price: maxPrice,
          category: category,
  ...
  const addToCart = (food) => {
    ...
    axios
      .post('http://localhost/php-rest-api/cart.php?action=add', item)
      .then((response) => {
  ...
  return (
    <>
      ...
      <div className="mx-[100px] mb-[30px] bg-slate-300 p-4">
        ...
        {/* Existing Filters */}
        <div className="filters flex gap-4 align-middle mt-4">
          ...
          />
          <Button text="Apply Filters" onClick={fetchData}>Apply filters</Button>
        </div>
      </div>
