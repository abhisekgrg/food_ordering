...
const FoodSingle = () => {
  ...
  const fetchRecommendations = async () => {
    try {
      ...
      if (response.data.status === true) {
        setRecommendations(response.data.data); // Save the recommended items
      }
    } catch (error) {
  ...
  useEffect(() => {
    fetchFood();
    fetchRecommendations(); // Fetch recommendations when component loads
  }, [id]);
      ...
      <div className="max-w-4xl mx-auto my-8 p-4 bg-gray-100 rounded-lg shadow-md">
        <div className="flex">
          ...
          <div className="w-1/2 pl-4">
            ...
            <h2 className="text-2xl font-semibold text-orange-500 mb-4">${food.price}</h2>
